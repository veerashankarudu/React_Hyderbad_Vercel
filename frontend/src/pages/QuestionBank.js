  ...
  return (
    <>
      ...
      <div className="page-container">
        ...
        {/* Semantic Search Bar */}
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center', margin: '0.75rem 0', padding: '10px 14px', background: 'rgba(124,58,237,0.05)', border: '1px solid rgba(124,58,237,0.15)', borderRadius: '10px', flexWrap: 'wrap' }}>
          <span style={{ fontSize: '0.8rem', fontWeight: 700, color: '#7C3AED', textTransform: 'uppercase', letterSpacing: '0.05em', whiteSpace: 'nowrap' }}>🧠 Semantic Search</span>
          <input
            type="text"
          ...
          <button
            ...
            style={{ padding: '7px 18px', background: 'linear-gradient(135deg,#7C3AED,#5B21B6)', color: '#fff', border: 'none', borderRadius: '7px', fontWeight: 600, fontSize: '0.88rem', cursor: 'pointer', opacity: (semanticLoading || !semanticQuery.trim()) ? 0.55 : 1 }}
            ...
            {(semanticResults.results || []).map(r => (
              <div key={r.id} onClick={() => navigate(`/mcq/${r.id}`)} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 10px', marginBottom: '4px', background: '#fff', border: '1px solid #e5e7eb', borderRadius: '8px', cursor: 'pointer', transition: 'border-color 0.15s' }}
                ...
                <div style={{ minWidth: '48px', textAlign: 'center' }}>
                  <div style={{ fontSize: '0.78rem', fontWeight: 700, color: '#7C3AED' }}>{Math.round((r.similarity || 0) * 100)}%</div>
                ...
                <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                  {r.techStack && <span style={{ fontSize: '0.75rem', padding: '2px 8px', background: '#ede9fe', color: '#5b21b6', borderRadius: '10px' }}>{r.techStack}</span>}
