...
import com.valkey.quizhub.repository.ChatMessageRepository;
...
public class ChatController {
    ...
    public ResponseEntity<List<Map<String, Object>>> sendMessage(
        ...
        // @bot → AI reply with chat history context + Breeth memory
        if (content.toLowerCase().contains("@bot")) {
            ...
            // Enrich with Breeth AI memory context (if available)
            try {
                List<Map<String, Object>> memoryContext = breethMemoryService.search(question, 5);
                if (!memoryContext.isEmpty()) {
                    StringBuilder memoryHints = new StringBuilder("[Memory context from Breeth AI]: ");
                    for (Map<String, Object> edge : memoryContext) {
                        Object fact = edge.get("fact");
                        if (fact != null) memoryHints.append(fact).append(". ");
                    }
                    history.add(0, memoryHints.toString());
                }
            } catch (Exception ignored) {
                // Breeth unavailable — continue without memory
            }

            String aiReply = aiService.chatReplyWithHistory(question, history);

            // Store this interaction in Breeth memory (async)
            try {
                breethMemoryService.writeEpisode(
                    sender.getFullName() + " asked: " + question + " | AI answered: " +
                    (aiReply.length() > 200 ? aiReply.substring(0, 200) : aiReply), false);
            } catch (Exception ignored) {}
