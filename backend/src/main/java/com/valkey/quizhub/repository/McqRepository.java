...
public interface McqRepository extends JpaRepository<Mcq, Long>, JpaSpecificationExecutor<Mcq> {
    ...
    @Query("SELECT m.reviewer.id, m.reviewer.fullName, m.reviewer.enterpriseId, COUNT(m) as cnt " +
           ...
           "AND (m.status = com.valkey.quizhub.enums.McqStatus.APPROVED OR m.status = com.valkey.quizhub.enums.McqStatus.REJECTED) " +
    ...
    @Query("SELECT m.reviewer.id, m.reviewer.fullName, m.reviewer.enterpriseId, " +
           "COUNT(m) as total, " +
           "SUM(CASE WHEN m.status = com.valkey.quizhub.enums.McqStatus.APPROVED THEN 1 ELSE 0 END) as approved, " +
           "SUM(CASE WHEN m.status = com.valkey.quizhub.enums.McqStatus.REJECTED THEN 1 ELSE 0 END) as rejected, " +
           "SUM(CASE WHEN m.status = com.valkey.quizhub.enums.McqStatus.UNDER_REVIEW THEN 1 ELSE 0 END) as pending " +
           "FROM Mcq m WHERE m.reviewer IS NOT NULL " +
           "GROUP BY m.reviewer.id, m.reviewer.fullName, m.reviewer.enterpriseId " +
           "ORDER BY total DESC")
