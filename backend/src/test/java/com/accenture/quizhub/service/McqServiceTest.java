...
class McqServiceTest {
    ...
    void setUp() {
        ...
        smeUser = User.builder().id(1L).enterpriseId("sme.user")
                .fullName("SME User").role(com.accenture.quizhub.enums.Role.SME).approved(true).build();

        adminUser = User.builder().id(2L).enterpriseId("admin.user")
                .fullName("Admin User").role(com.accenture.quizhub.enums.Role.ADMIN).approved(true).build();
    ...
    void createMcq_unknownTechStack_throwsResourceNotFound() {
        ...
        assertThrows(com.accenture.quizhub.exception.ResourceNotFoundException.class,
                () -> mcqService.createMcq(validRequest, smeUser));
    }
    ...
    void updateMcq_byNonCreatorNonAdmin_throwsBadRequest() {
        User other = User.builder().id(99L).enterpriseId("other").role(com.accenture.quizhub.enums.Role.SME).build();
        Mcq existing = buildMcq(1L, McqStatus.DRAFT);
        existing.setCreator(smeUser);
        when(mcqRepository.findById(1L)).thenReturn(Optional.of(existing));
    ...
    void createMcq_duplicateNoExistingId_messageLacksDuplicatePrefix() {
        ...
        com.accenture.quizhub.exception.BadRequestException ex =
                assertThrows(com.accenture.quizhub.exception.BadRequestException.class,
                        () -> mcqService.createMcq(validRequest, smeUser));
